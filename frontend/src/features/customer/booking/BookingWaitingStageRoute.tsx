...
export default function BookingWaitingStageRoute({
  expectedStage,
}: {
  expectedStage: BookingStage;
}) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToBooking(
      ...
      (value) => {
        ...
        // The existing waiting screen already owns the completed -> billing redirect.
        ...
        router.replace(
          routeForStage(bookingId, targetStage),
        );
      },
      (listenerError) => {
        console.error(
          "[BookingWaitingStageRoute] listener failed:",
          listenerError,
        );
      },
    );
